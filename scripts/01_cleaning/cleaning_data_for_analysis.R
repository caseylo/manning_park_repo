...
write.csv(data_2025_clean, file = "data/cleaned/data_2025_select.csv")
...
write.csv(BEC_clean, file = "data/cleaned/BEC_select.csv")
...
## Adding a column to 2025 data with year since original survey

data_2025_clean_filt <- data_2025_clean_filt %>%
...
write.csv(data_2025_long, file = "data/cleaned/data_2025_long.csv")
...
write.csv(BEC_data_long, file = "data/cleaned/BEC_data_long.csv")
...
write.csv(model_data, file = "data/processed/model_data.csv")

####

##Sanity checks
table(BEC_no_func$Functional_group)
table(data_2025$Functional_group)
table(data_2025$`2019Plot`)
table(data_2025$PlotNumber)
table(BEC_no_func$PlotNumber)
table(data_2025$Species)
table(BEC_no_func$Species)

table(unique(BEC_clean$PlotNumber))
length(unique(BEC_clean$PlotNumber))

table(unique(data_2025_clean$PlotNumber))
length(unique(data_2025_clean$PlotNumber))

length(unique(BEC_clean$Species))
length(unique(data_2025_clean$Species))
